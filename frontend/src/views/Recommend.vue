  ...
  <div class="space-y-4">
    <section class="bg-gradient-to-r from-brand to-brand-dark text-white rounded-2xl p-6 shadow-md relative overflow-hidden">
      ...
      <div class="relative z-10 max-w-2xl">
        <span class="bg-white/20 text-white text-xs px-3 py-1 rounded-full font-bold inline-block mb-2">살만해 AI 솔루션</span>
        <h2 class="text-2xl font-bold mb-1">종합 치안 등급 & 가성비 맞춤형 매물 추천</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
      <aside class="md:col-span-4 bg-white rounded-2xl border border-slate-200 p-4 shadow-sm space-y-4">
        <h3 class="font-bold text-slate-900 border-b border-slate-100 pb-2">🎚️ 내 선호도 가중치</h3>
      ...
      <section class="md:col-span-8 space-y-3">
        <article v-for="(property, index) in scoredProperties" :key="property.id" class="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-brand-light border-2 border-brand/30 rounded-2xl flex flex-col items-center justify-center text-brand-dark font-bold shrink-0">
              <span class="text-[10px] text-slate-500 font-bold">적합도</span>
              <span class="text-lg">{{ property.finalScore }}%</span>
            </div>
            <div>
              <div class="flex flex-wrap items-center gap-2">
                <span class="bg-brand-light text-brand-dark text-[10px] font-bold px-2 py-0.5 rounded-full">AI 추천 {{ index + 1 }}위</span>
                <span class="text-xs text-slate-400">{{ property.address }}</span>
              </div>
              <h4 class="font-bold text-slate-950 text-base mt-1">{{ property.name }} <span class="text-xs font-normal text-slate-500">({{ property.type }})</span></h4>
          ...
          <button @click="selectOnMap(property)" class="bg-brand hover:bg-brand-dark text-white font-bold text-xs px-4 py-2.5 rounded-xl transition">위치 확인</button>
        </article>
      </section>
    </div>
  </div>
