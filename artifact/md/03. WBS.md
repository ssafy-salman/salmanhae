...
| 3 | DB/데이터 파이프라인 | migration, 실거래가/안전시설 스키마, 법률 chunk 스키마 | 완료 |
| 4 | Backend API | 인증, 매물, 지도 viewport, 안전, 시세, 챗봇 프록시 | 완료 |
| 5 | AI Backend | LangGraph supervisor, 워커, RAG, Spring 연동 | 완료 |
| 6 | Frontend | 지도 탐색, 인증, 챗봇, 분석 카드 화면 | 완료 |
| 7 | 테스트/QA | 단위/통합 테스트, phase QA | 완료 |
| 8 | 배포/운영 | Cloud Run, env, deployment guide | 문서화 완료/운영 준비 |
| 9 | 제출 산출물 | 요구사항정의서, UseCase, WBS, ERD, API, Class, 화면정의서, 발표/시연 구성안 | 완료 |
...
| 7.1 | Backend 테스트 | Controller/Service/DAO 테스트 | QA | JUnit tests | 완료 |
| 7.2 | AI 테스트 | supervisor, RAG, Spring client 테스트 | QA | pytest tests | 완료 |
| 7.3 | Frontend 테스트 | API/store/util 테스트 | QA | Vitest tests | 완료 |
| 8.1 | 배포 문서 | Cloud Run, 환경변수, 운영 절차 | Infra | `docs/14_DEPLOYMENT_GUIDE.md` | 완료 |
| 9.1 | 제출 문서 정리 | 산출물 Markdown 최종본 작성 | 문서 | `artifact/md` | 완료 |
| 9.2 | 참고 명세 대응표 | SSAFY HOME 필수/평가 기준과 살만해 구현 범위 매핑 | 문서 | `10_REFERENCE_ALIGNMENT.md` | 완료 |
| 9.3 | 제출 패키지 정리 | 설계 문서, Schema SQL, Spring/Vue/AI 소스, 활용 데이터셋, PPT/PDF, AI 사용 보고서 | 제출 | GitLab/zip/PPT | 완료 |

## 3. 제출 전 체크리스트

| 항목 | 체크 |
| --- | --- |
| 요구사항과 구현 범위가 일치하는가 | 완료 |
| MVP 제외 기능이 구현 완료처럼 적히지 않았는가 | 완료 |
| API 명세와 Controller endpoint가 일치하는가 | 완료 |
| ERD가 Supabase schema를 반영하는가 | 완료 |
| 발표/시연 흐름이 3분 안에 가능한가 | 완료 |
| 평가표의 완성도/편리성/AI활용성/발표/팀웍 기준을 발표 자료에 반영했는가 | 완료 |
